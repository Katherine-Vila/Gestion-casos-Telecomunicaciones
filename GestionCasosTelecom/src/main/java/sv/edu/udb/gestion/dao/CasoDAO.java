...
public class CasoDAO {
    // ------------------------------------------------------------
    // Base de datos / DAO (Persona 1 + adaptaciones Persona 3)
    // - Aqui van consultas SQL sobre la tabla 'casos'
    // ------------------------------------------------------------
    ...
    /**
     * Cuenta casos por estado dentro de un rango de fechas (inclusive).
     * La fecha del caso se toma desde la columna {@code fecha_solicitud}.
     */
    public Map<EstadoCaso, Integer> contarCasosPorEstadoYFechas(Date fechaInicio, Date fechaFin) throws SQLException {
        Map<EstadoCaso, Integer> conteo = new EnumMap<>(EstadoCaso.class);
        // Estados del reporte solicitado
        conteo.put(EstadoCaso.FINALIZADO, 0);
        conteo.put(EstadoCaso.EN_DESARROLLO, 0);
        conteo.put(EstadoCaso.RECHAZADO, 0);
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    // Estado no contemplado para el reporte
                }
            }
        }
